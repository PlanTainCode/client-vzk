...
const Illnesses = ({ submenuones, submenutwos, category, categories }) => {
  ...
  return (
    <>
      <Head>
        <title>{category.attributes.Title} | ВЗК</title>
        <meta property="og:title" content={category.attributes.Title} />
        <meta property="og:site_name" content={category.attributes.Title} />
        <meta property="og:description" content={category.attributes.Description} />
      </Head>
        ...
        <main className='main ta'>
                  ...
                  <section className="ta-grid">
                      <div className="container ta-grid__inner">
                          {category.attributes.subills.data.map((item) => 
                            <Link href={`subillnesses/${item.attributes.slug}`} class="article">
...
export async function getStaticPaths() {
  const submenuRes = await fetchAPI("/submenuones", { fields: ["slug"] });
  return {
    paths: submenuRes.data.map((sub) => ({
      params: {
...
export async function getStaticProps({ params }) {
  const matchingCategories = await fetchAPI("/submenuones", {
